...
export default function PaymentReviewPage() {
  ...
  async function load() {
    ...
    try {
      ...
      setItems((data.items || []).filter((item) => item.source === "contact_request" && item.paymentStatus === "UNDER_REVIEW"));
    } catch (err) {
  ...
  async function confirmPayment(item: Item) {
    ...
    setError("");
    try {
      const response = await fetch(`${API_URL}/contact-requests/${encodeURIComponent(item.id)}/confirm-payment`, {
        method: "POST",
        credentials: "include",
      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col gap-4 rounded-3xl border border-border bg-card p-5 shadow-sm sm:flex-row sm:items-center sm:justify-between sm:p-7">
          <div>
            ...
            <p className="mt-2 text-sm leading-7 text-muted-foreground">الطلبات التي أعلن أصحابها عن التحويل وتحتاج تأكيد الموظف أو المحاسب.</p>
          </div>
        ...
        <section className="mt-6 rounded-3xl border border-border bg-card p-5 shadow-sm sm:p-7">
          ...
          <div className="grid gap-4">
            {items.map((item) => (
              <article key={item.id} className="rounded-2xl border border-border bg-background p-4 sm:p-5">
                <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <div className="mt-2 text-sm font-bold">{formatAmount(item)}</div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <Button asChild type="button" variant="outline"><Link href={`/admin-dashboard.html?customerRequest=${encodeURIComponent(item.id)}`}><ExternalLink className="size-4" />فتح الطلب</Link></Button>
                  </div>
                </div>
              </article>
            ))}
