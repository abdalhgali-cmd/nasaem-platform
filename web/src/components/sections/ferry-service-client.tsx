...
import { Anchor, CalendarDays, CheckCircle2, Ship, Users } from "lucide-react";
...
export function FerryServiceClient() {
  ...
  React.useEffect(() => {
    fetch(`${API_URL}/services/public`)
      ...
      .catch(() => setError("تعذر تحميل خدمة العبارات، حاول تحديث الصفحة."))
    ...
    try {
      const response = await fetch(`${API_URL}/contact-requests`, {
        ...
        body: JSON.stringify({
          ...
          message: `طلب حجز عبارة: ${form.route} بتاريخ ${form.travelDate} لعدد ${form.travelers} مسافر. ${form.notes}`,
        }),
      });
      const payload = await response.json();
      if (!response.ok || !payload.success) throw new Error(payload.message || "تعذر إرسال طلب حجز العبارة");
      setSuccess(`تم استلام الطلب بنجاح. رقم الطلب: ${payload.data?.id ?? "سيظهر في المتابعة"}`);
    } catch (err) {
      ...
      <section className="bg-primary py-16 text-primary-foreground">
        <Container>
          <div className="mx-auto max-w-4xl text-center">
            <div className="mx-auto flex size-14 items-center justify-center rounded-2xl bg-white/10"><Ship className="size-7" /></div>
            <p className="mt-5 text-sm font-bold text-secondary">الرحلات البحرية</p>
            <h1 className="mt-2 text-3xl font-black sm:text-5xl">احجز رحلتك بالعبارة</h1>
            <p className="mx-auto mt-4 max-w-2xl text-sm leading-7 text-primary-foreground/75 sm:text-base">
              أرسل بيانات الرحلة والعدد والتاريخ، وسيقوم فريق نسائم الحرمين بمتابعة التوفر والإجراءات معك.
            </p>
          </div>
        </Container>
      </section>
