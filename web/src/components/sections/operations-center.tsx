"use client";
/* eslint-disable react-hooks/set-state-in-effect -- initial queue data loading synchronizes with the API. */

import * as React from "react";
import Link from "next/link";
import { AlertCircle, ArrowLeft, BarChart3, CheckCircle2, ClipboardList, CreditCard, ExternalLink, FileText, RefreshCw, Search, SlidersHorizontal, UserRound } from "lucide-react";
...
function actionTarget(item: OperationItem) { return item.source === "order" ? `/admin-dashboard.html?order=${encodeURIComponent(item.id)}` : `/admin/cases?requestId=${encodeURIComponent(item.id)}`; }
    ...
    <main className="min-h-screen bg-section py-8 sm:py-10">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col gap-4 rounded-3xl border border-border bg-card p-5 shadow-sm sm:flex-row sm:items-center sm:justify-between sm:p-7">
          ...
          <div className="flex flex-wrap gap-2"><Button type="button" variant="outline" onClick={() => void load()} disabled={loading}><RefreshCw className={loading ? "size-4 animate-spin" : "size-4"} />تحديث</Button><Button asChild variant="gold"><Link href="/admin/reports"><BarChart3 className="size-4" />التقارير</Link></Button></div>
        </div>
        ...
        <section className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">{queueCards.map(({ key, label: cardLabel, icon: Icon, tone }) => {
          const cardFilter = key === "waitingPaymentReview" ? "PAYMENT" : key === "unassignedOrders" ? "UNASSIGNED" : key === "waitingCustomer" ? "CUSTOMER" : key === "readyToDeliver" ? "URGENT" : "ALL";
          const selected = filter === cardFilter && cardFilter !== "ALL";
          return <button type="button" key={key} onClick={() => setFilter(cardFilter)} className={`rounded-2xl border bg-card p-5 text-start shadow-sm transition hover:-translate-y-1 hover:shadow-md ${selected ? "border-primary ring-2 ring-primary/15" : "border-border"}`}><div className={`flex size-10 items-center justify-center rounded-xl ${tone}`}><Icon className="size-5" /></div><p className="mt-4 text-xs font-bold text-muted-foreground">{cardLabel}</p><p className="mt-1 text-3xl font-black text-foreground">{data?.queues?.[key] ?? 0}</p></button>;
        })}</section>
        <section className="mt-6 rounded-3xl border border-border bg-card p-5 shadow-sm sm:p-7">
          ...
          <div className="mt-5 flex gap-3"><div className="relative flex-1"><Search className="pointer-events-none absolute start-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="بحث بالمرجع أو الاسم أو الهاتف أو الخدمة" className="h-11 w-full rounded-xl border border-border bg-background ps-10 pe-3 text-sm outline-none transition focus:border-primary" /></div><Button type="button" variant="outline" onClick={() => setShowFilters((value) => !value)}><SlidersHorizontal className="size-4" />الفلاتر</Button></div>
          {showFilters ? <div className="mt-3 grid gap-3 lg:grid-cols-3">
          ...
          {showFilters ? <div className="mt-3 grid gap-3 sm:grid-cols-3">
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-3"><Button asChild variant="outline" className="h-12 justify-center"><Link href="/admin/cases">مساحة عمل الطلبات <ArrowLeft className="size-4" /></Link></Button><Button asChild variant="outline" className="h-12 justify-center"><Link href="/admin/payment-review">مراجعة الدفعات <ArrowLeft className="size-4" /></Link></Button><Button asChild variant="outline" className="h-12 justify-center"><Link href="/admin/reports">عرض التقارير <ArrowLeft className="size-4" /></Link></Button></div>
      </div>
    </main>
  );
}
