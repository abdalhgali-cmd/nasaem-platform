...
import { AppButton, BrandHeader, FormField, StepIndicator, SurfaceCard } from "../../src/components/ui";
import { colors } from "../../src/theme";

const REQUEST_STEPS=["الخدمة","المسافر","المستندات","المراجعة"];
...
export default function ServiceRequest(){
 ...
 {loadingReq?<ActivityIndicator color={colors.navy}/>:caseRequirements.length>0&&<View style={s.requirements}><Text style={s.sectionTitle}>المتطلبات والمستندات</Text>{caseRequirements.map(r=><RequirementField key={r.id} req={r} answer={answers[r.id]??""} file={docs[r.id]??null} onAnswer={v=>setAnswers(a=>({...a,[r.id]:v}))} onPick={()=>pickRequirement(r)} onRemove={()=>setDocs(d=>({...d,[r.id]:null}))}/>)}</View>}{!!error&&<Text style={s.error}>{error}</Text>}<AppButton disabled={!complete} label="مراجعة الطلب" onPress={()=>setReview(true)}/></ScrollView></SafeAreaView>;
}
...
const s=StyleSheet.create({safe:{flex:1,backgroundColor:colors.background},page:{padding:16,gap:14,paddingBottom:40},formCard:{gap:13},successPage:{flex:1,padding:20,justifyContent:"center"},successCard:{alignItems:"center",gap:13,paddingVertical:28},successBubble:{width:64,height:64,borderRadius:32,backgroundColor:colors.successSoft,alignItems:"center",justifyContent:"center"},successTitle:{fontSize:19,fontWeight:"900",color:colors.text,textAlign:"center"},full:{alignSelf:"stretch"},title:{fontSize:20,fontWeight:"900",color:colors.navy,textAlign:"right"},desc:{fontSize:11.5,color:colors.muted,textAlign:"center",lineHeight:19},label:{fontSize:11,color:colors.muted,textAlign:"right",marginBottom:5},input:{backgroundColor:"#FFF",borderWidth:1,borderColor:colors.border,borderRadius:12,padding:12,fontSize:12.5,minHeight:48},readonly:{backgroundColor:"#F1F3F5",color:colors.muted},primary:{backgroundColor:colors.navy,borderRadius:12,padding:14,flex:2},primaryText:{color:"#FFF",fontWeight:"800",textAlign:"center"},disabled:{opacity:.4},review:{backgroundColor:"#FFF",borderWidth:1,borderColor:colors.border,borderRadius:14,padding:14},value:{fontSize:13,fontWeight:"700",color:colors.text,textAlign:"right"},note:{fontSize:11,color:colors.subtle,textAlign:"right",lineHeight:18},actions:{flexDirection:"row-reverse",gap:10},outline:{borderWidth:1.5,borderColor:colors.navy,borderRadius:10,padding:13,flex:1},outlineText:{color:colors.navy,fontWeight:"800",textAlign:"center"},error:{color:colors.danger,textAlign:"right",fontSize:11},successIcon:{fontSize:34,color:colors.success,textAlign:"center",fontWeight:"900"},requestBox:{alignSelf:"stretch",backgroundColor:colors.soft,borderWidth:1,borderColor:colors.border,borderRadius:14,padding:18},requestId:{fontFamily:"monospace",fontSize:17,fontWeight:"900",color:colors.navy,textAlign:"center",marginTop:6},requirements:{gap:10,marginTop:4},sectionTitle:{fontSize:15,fontWeight:"900",color:colors.navy,textAlign:"right"},reqCard:{backgroundColor:"#FFF",borderWidth:1,borderColor:colors.border,borderRadius:14,padding:13,gap:7},reqTitle:{fontSize:12,fontWeight:"800",color:colors.text,textAlign:"right"},reqDesc:{fontSize:10.5,color:colors.muted,textAlign:"right",lineHeight:17},docButton:{backgroundColor:colors.navy,borderRadius:10,padding:12},docButtonText:{color:"#FFF",fontWeight:"700",textAlign:"center",fontSize:11},docDone:{backgroundColor:colors.successSoft},docDoneText:{color:colors.success,fontWeight:"700",textAlign:"center",fontSize:11},chips:{flexDirection:"row-reverse",flexWrap:"wrap",gap:8},chip:{borderWidth:1,borderColor:colors.border,borderRadius:999,paddingHorizontal:12,paddingVertical:8},chipActive:{backgroundColor:colors.navy,borderColor:colors.navy},chipText:{color:colors.text,fontSize:11},chipTextActive:{color:"#FFF",fontSize:11,fontWeight:"700"},card:{gap:10,marginTop:10},cardHead:{flexDirection:"row-reverse",justifyContent:"space-between",alignItems:"center"},cardTitle:{fontSize:13,fontWeight:"800",color:colors.navy,textAlign:"right"},remove:{color:colors.danger,fontSize:11,fontWeight:"700"},two:{flexDirection:"row-reverse",gap:8},flex:{flex:1},add:{borderWidth:1.5,borderStyle:"dashed",borderColor:colors.navy,borderRadius:12,padding:13,marginTop:10},addText:{color:colors.navy,fontWeight:"800",textAlign:"center"},line:{fontSize:12,color:colors.muted,textAlign:"right"},ok:{fontSize:10,color:colors.success,textAlign:"right",marginTop:2}});
