...
const makeTraveler=(n:number):Traveler=>({
  ...
  nationality:"",
  guarantorNumber:"",
  docs:{passport:null,guarantorId:null},
});
...
export default function UmrahScreen(){
  ...
  const complete=useMemo(
    ()=>phone.trim().length>=6 && travelers.every(t=>t.name.trim()&&t.passport.trim()&&t.nationality.trim()&&t.guarantorNumber.trim()&&t.docs.passport&&t.docs.guarantorId),
    [travelers,phone],
  );
  ...
  async function pickDocument(id:string,key:keyof TravelerDocs,label:string){
    ...
    setTravelers(current=>current.map(t=>t.id===id?{
      ...t,
      docs:{...t.docs,[key]:{uri:asset.uri,name:asset.name,mimeType:asset.mimeType,label}}
    }:t));
  }

  function removeDocument(id:string,key:keyof TravelerDocs){
    setTravelers(current=>current.map(t=>t.id===id?{...t,docs:{...t.docs,[key]:null}}:t));
  }
  ...
  async function submit(){
    try{
      ...
      travelers.forEach((t,index)=>{
        if(t.docs.passport) documents.push({...t.docs.passport,travelerIndex:index});
        if(t.docs.guarantorId) documents.push({...t.docs.guarantorId,travelerIndex:index});
      });
      ...
      setError(e instanceof Error?e.message:"تعذر إرسال طلب العمرة");
    }finally{
      setBusy(false);
    }
  }
...
const s=StyleSheet.create({
 safe:{flex:1,backgroundColor:colors.background},page:{padding:20,gap:14,paddingBottom:40},successPage:{flex:1,padding:26,justifyContent:"center",gap:14},
 title:{fontSize:20,fontWeight:"800",color:colors.navy,textAlign:"right"},desc:{fontSize:12,color:colors.muted,textAlign:"right"},card:{backgroundColor:"#FFF",borderWidth:1,borderColor:colors.border,borderRadius:14,padding:14,gap:10},cardHead:{flexDirection:"row-reverse",justifyContent:"space-between"},cardTitle:{fontSize:13,fontWeight:"800",color:colors.navy,textAlign:"right"},remove:{color:colors.danger,fontSize:11,fontWeight:"700"},input:{backgroundColor:colors.soft,borderWidth:1,borderColor:colors.border,borderRadius:9,padding:11,fontSize:12.5},two:{flexDirection:"row-reverse",gap:8},flex:{flex:1},
 doc:{backgroundColor:colors.soft,borderRadius:9,padding:10,flexDirection:"row-reverse",justifyContent:"space-between",alignItems:"center",gap:8},docInfo:{flex:1},docLabel:{fontSize:11.5,color:colors.text,textAlign:"right"},pending:{fontSize:9.5,color:colors.subtle,textAlign:"right",marginTop:2},ok:{fontSize:10,color:colors.success,textAlign:"right",marginTop:4},docButton:{backgroundColor:colors.navy,borderRadius:999,paddingHorizontal:13,paddingVertical:7},docButtonText:{color:"#FFF",fontSize:10.5,fontWeight:"700"},docDone:{backgroundColor:"#E8F7ED"},docDoneText:{color:colors.success,fontSize:10.5,fontWeight:"700"},
 add:{borderWidth:1.5,borderStyle:"dashed",borderColor:colors.navy,borderRadius:10,padding:12},addText:{color:colors.navy,fontWeight:"800",textAlign:"center"},primary:{backgroundColor:colors.navy,borderRadius:10,padding:14,flex:2},primaryText:{color:"#FFF",fontWeight:"800",textAlign:"center"},disabled:{opacity:.4},note:{fontSize:11,color:colors.subtle,lineHeight:18,textAlign:"right"},actions:{flexDirection:"row-reverse",gap:10},outline:{borderWidth:1.5,borderColor:colors.navy,borderRadius:10,padding:13,flex:1},outlineText:{color:colors.navy,fontWeight:"800",textAlign:"center"},line:{fontSize:12,color:colors.muted,textAlign:"right"},error:{fontSize:11,color:colors.danger,textAlign:"right"},
