...
const makeTraveler=(n:number):Traveler=>({id:`traveler-${Date.now()}-${n}`,name:"",passport:"",nationality:"",guarantorNumber:"",docs:{}});
...
export default function UmrahScreen(){
 ...
 useEffect(()=>{
   let active=true;
   if(!requirementServiceId){setRequirements([]);return()=>{active=false};}
 ...
 const travelerDocumentRequirements=useMemo(()=>requirements.filter(r=>(r.type??"DOCUMENT")==="DOCUMENT"&&r.scope==="TRAVELER"),[requirements]);
 const update=(id:string,patch:Partial<Traveler>)=>setTravelers(v=>v.map(t=>t.id===id?{...t,...patch}:t));
 const complete=useMemo(()=>phone.trim().length>=6&&!requirementsLoading&&(!packages.length||Boolean(selectedPackageId))&&travelers.every(t=>t.name.trim()&&t.passport.trim()&&t.nationality.trim()&&t.guarantorNumber.trim()&&travelerDocumentRequirements.filter(r=>r.required).every(r=>Boolean(t.docs[r.id]))),[travelers,phone,requirementsLoading,packages.length,selectedPackageId,travelerDocumentRequirements]);

 async function pickDocument(id:string,requirementOrLegacyKey:PublicRequirement|string,legacyLabel?:string){
   const legacyIndex=requirementOrLegacyKey==="passport"?0:1;
   const requirement=typeof requirementOrLegacyKey==="string"?travelerDocumentRequirements[legacyIndex]:requirementOrLegacyKey;
   if(!requirement){setError("هذا المستند غير منشور ضمن متطلبات العمرة الحالية.");return;}
   const types=requirement.allowedMimeTypes?.length?requirement.allowedMimeTypes:["image/jpeg","image/png","image/webp","application/pdf"];
   const result=await DocumentPicker.getDocumentAsync({type:types,copyToCacheDirectory:true,multiple:false});
   if(result.canceled)return;
   const asset=result.assets[0];
   const document:UploadAsset={uri:asset.uri,name:asset.name,mimeType:asset.mimeType,label:requirement.name||legacyLabel||"مستند مطلوب",requirementId:requirement.id};
   setTravelers(current=>current.map(t=>t.id===id?{...t,docs:{...t.docs,[requirement.id]:document,...(typeof requirementOrLegacyKey==="string"?{[requirementOrLegacyKey]:document}:{})}}:t));
 }
 function removeDocument(id:string,requirementId:string){
   const linkedRequirement=requirementId==="passport"?travelerDocumentRequirements[0]:requirementId==="guarantorId"?travelerDocumentRequirements[1]:undefined;
   setTravelers(current=>current.map(t=>t.id===id?{...t,docs:{...t.docs,[requirementId]:null,...(linkedRequirement?{[linkedRequirement.id]:null}:{})}}:t));
 }
 ...
 async function submit(){
   try{
     ...
     travelers.forEach((t,index)=>travelerDocumentRequirements.forEach(requirement=>{const document=t.docs[requirement.id];if(document)documents.push({...document,requirementId:requirement.id,travelerIndex:index,label:requirement.name});}));
