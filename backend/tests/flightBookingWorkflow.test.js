...
import { loginAsSuperAdmin, request, uniqueSuffix } from "./helpers/api.js";
...
describe("flight booking workflow", () => {
  ...
  before(async () => {
    ...
    const flightRes = await admin.post("/api/flights").send({
      airline: "TARCO",
      flightNumber: `3T${uniqueSuffix().slice(-4)}`,
      originCode: "PZU",
      originName: "Port Sudan",
      destinationCode: "JED",
      destinationName: "Jeddah",
      departureAt: "2026-09-15T08:30:00+02:00",
      arrivalAt: "2026-09-15T11:30:00+02:00",
      stops: 0,
      baggage: "30 KG",
      cabin: "Economy",
      price: 1500000,
    ...
    const bankRes = await admin.post("/api/flight-bookings/admin/bank-accounts").send({
      key: `test-bank-${uniqueSuffix()}`,
      label: "حساب اختبار الطيران",
      bankName: "Test Bank",
  ...
  test("runs request -> provisional -> payment receipt -> confirmation -> final ticket", async () => {
    const createRes = await request(request.app)
      .post("/api/flight-bookings")
      .send({
        flightId,
        amount: 1500000,
        currency: "SDG",
        contact: { fullName: "Flight Workflow Test", phone, email: "flight@test.local" },
        passengers: [{
          firstName: "TEST",
          lastName: "PASSENGER",
  ...
  test("rejects payment receipt upload before provisional ticket", async () => {
    const createRes = await request(request.app)
      ...
      .send({
        ...
        amount: 1500000,
        currency: "SDG",
        contact: { fullName: "Early Receipt Test", phone: `24992${uniqueSuffix().slice(-7)}` },
        passengers: [{ firstName: "EARLY", lastName: "TEST", nationality: "Sudan", passportNo: `P${uniqueSuffix()}` }],
      });
    assert.equal(createRes.status, 201);

    const res = await request(request.app)
      .post(`/api/flight-bookings/${createRes.body.booking.booking_number}/payment-receipt`)
      .field("phone", createRes.body.booking.customer_phone || "wrong")
