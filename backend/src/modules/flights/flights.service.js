...
async function getFxRates() {
  const rows = await prisma.setting.findMany({
    where: { key: { in: Object.values(FX_KEYS) } },
  });

  return Object.fromEntries(
    Object.entries(FX_KEYS).map(([code, key]) => {
...
async function refreshStoredFxPrices(rates) {
  for (const currency of Object.keys(FX_KEYS)) {
    const rate = Number(rates[currency] ?? 0);
    if (!rate) {
      await prisma.$executeRawUnsafe(
        `UPDATE flight_inventory SET fx_rate_to_sdg = NULL, price_sdg = NULL, updated_at = CURRENT_TIMESTAMP WHERE currency = $1 AND source = 'MANUAL'`,
        currency,
      );
      continue;
    }
    ...
    await prisma.$executeRawUnsafe(
      `UPDATE flight_inventory
       SET fx_rate_to_sdg = $2,
           price_sdg = price * $2,
           updated_at = CURRENT_TIMESTAMP
       WHERE currency = $1 AND source = 'MANUAL'`,
      currency,
      rate,
    );
  }
  ...
  await prisma.$executeRawUnsafe(
    `UPDATE flight_inventory
     SET fx_rate_to_sdg = 1,
         price_sdg = price,
         updated_at = CURRENT_TIMESTAMP
     WHERE currency = 'SDG' AND source = 'MANUAL'`,
  );
}

export async function updateCurrencyRates(input) {
  for (const [code, key] of Object.entries(FX_KEYS)) {
    if (input[code] === undefined) continue;

    const value = Number(input[code]);
    if (!Number.isFinite(value) || value < 0) {
      throw new Error(`Invalid FX rate for ${code}`);
    }
  ...
  await refreshStoredFxPrices(rates);
  return rates;
}

function convertToSdg(price, currency, rates) {
  if (currency === "SDG") {
    return { rate: 1, priceSdg: Number(price) };
  }
...
function mapRow(row) {
  return {
    ...
    priceSdg: row.price_sdg != null ? Number(row.price_sdg) : null,
...
export async function listManualFlights({ page = 1, limit = 50, activeOnly = false } = {}) {
  ...
  const where = activeOnly ? "WHERE active = true AND source = 'MANUAL'" : "WHERE source = 'MANUAL'";
  ...
  const countRows = await prisma.$queryRawUnsafe(
    `SELECT COUNT(*)::int AS count FROM flight_inventory ${where}`,
  );
...
export async function createManualFlight(input) {
  ...
  if (!departureAt || !arrivalAt || arrivalAt <= departureAt) {
    throw new Error("Invalid departure/arrival time");
  }
  ...
  const durationMinutes = input.durationMinutes
    ...
    : Math.round((arrivalAt - departureAt) / 60000);
  ...
  await prisma.$executeRawUnsafe(
    ...
    id,
    normalizeText(input.airline),
    normalizeText(input.flightNumber),
    normalizeText(input.originCode) || null,
    normalizeText(input.originName),
    normalizeText(input.destinationCode) || null,
    normalizeText(input.destinationName),
    departureAt,
    arrivalAt,
    durationMinutes,
    Number(input.stops || 0),
    normalizeText(input.baggage) || null,
    normalizeText(input.cabin) || null,
    Number(input.price),
    currency,
    rate,
    priceSdg,
    input.availableSeats == null ? null : Number(input.availableSeats),
    normalizeText(input.externalRef) || null,
    input.active !== false,
  );

  const rows = await prisma.$queryRawUnsafe(
    `SELECT * FROM flight_inventory WHERE id = $1`,
    id,
  );
...
export async function updateManualFlight(id, input) {
  const existing = await prisma.$queryRawUnsafe(
    `SELECT * FROM flight_inventory WHERE id = $1 AND source = 'MANUAL'`,
    id,
  );
  ...
  const rates = await getFxRates();
  const currency = String(merged.currency).toUpperCase();
  const { rate, priceSdg } = convertToSdg(merged.price, currency, rates);
  ...
  await prisma.$executeRawUnsafe(
    `UPDATE flight_inventory SET airline_name=$2, flight_number=$3, origin_code=$4, origin_name=$5,
      ...
      external_ref=$19, active=$20, updated_at=CURRENT_TIMESTAMP WHERE id=$1 AND source='MANUAL'`,
    id,
    merged.airline,
    merged.flightNumber,
    merged.originCode || null,
    merged.originName,
    merged.destinationCode || null,
    merged.destinationName,
    departureAt,
    arrivalAt,
    Number(merged.durationMinutes || Math.round((arrivalAt - departureAt) / 60000)),
    Number(merged.stops || 0),
    merged.baggage || null,
    merged.cabin || null,
    Number(merged.price),
    currency,
    rate,
    priceSdg,
    merged.availableSeats == null ? null : Number(merged.availableSeats),
    merged.externalRef || null,
...
export async function deleteManualFlight(id) {
  await prisma.$executeRawUnsafe(
    `DELETE FROM flight_inventory WHERE id = $1 AND source = 'MANUAL'`,
    id,
  );
}

function buildRouteWhere(leg) {
  const conditions = ["active = true", "source = 'MANUAL'", "departure_at >= $1", "departure_at < $2"];
  const params = [normalizeDate(leg.departureDate), normalizeDate(leg.departureDate, true)];

  if (leg.from) {
    conditions.push(
      `(LOWER(origin_code) = LOWER($3) OR LOWER(origin_name) = LOWER($3))`,
    );
  ...
  if (leg.to) {
    ...
    conditions.push(
      `(LOWER(destination_code) = LOWER($${index}) OR LOWER(destination_name) = LOWER($${index}))`,
    );
...
export async function searchManualFlights(legs) {
  ...
  for (const leg of legs) {
    ...
    const rows = await prisma.$queryRawUnsafe(
      `SELECT * FROM flight_inventory WHERE ${where}
       ORDER BY price_sdg NULLS LAST, departure_at ASC LIMIT 100`,
      ...params,
    );
