...
router.post("/:id/confirm-payment", requireRole("SUPER_ADMIN", "ADMIN", "ACCOUNTANT"), async (req, res, next) => { try { res.json({ success: true, booking: await confirmPayment(req.params.id, req.user?.id, req.body.note, req.user.organizationId) }); } catch (e) { next(e); } });
router.post("/:id/final-ticket", requireRole("SUPER_ADMIN", "ADMIN", "EMPLOYEE"), upload.single("file"), async (req, res, next) => { try { res.json({ success: true, booking: await issueFinalTicket(req.params.id, req.file, req.user.organizationId) }); } catch (e) { next(e); } });
