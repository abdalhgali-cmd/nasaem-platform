...
// POST /api/flight-bookings is unauthenticated, so a client-supplied
// customerId is never trusted on its own — anyone could pass an arbitrary
// existing id and attach a fabricated booking/order to a stranger's
// record. Only reused once the request's own contact phone matches that
// customer's phone on file, the same ownership check this file already
...
export async function getFlightBooking(idOrNumber) { const rows = await prisma.$queryRawUnsafe(`SELECT fb.*, c."fullName" AS customer_name, c.phone AS customer_phone, c.email AS customer_email FROM flight_bookings fb JOIN "Customer" c ON c.id=fb.customer_id WHERE fb.id=$1 OR fb.booking_number=$1 LIMIT 1`, idOrNumber); if (!rows[0]) return null; return mapBookingRow(rows[0]); }
...
export async function listFlightBookings(status) { const rows = await prisma.$queryRawUnsafe(`SELECT fb.*, c."fullName" AS customer_name, c.phone AS customer_phone, c.email AS customer_email FROM flight_bookings fb JOIN "Customer" c ON c.id=fb.customer_id ${status ? "WHERE fb.status=$1" : ""} ORDER BY fb.created_at DESC LIMIT 200`, ...(status ? [status] : [])); return rows.map(mapBookingRow); }
export async function uploadProvisionalTicket(id, file) { const booking = await getFlightBooking(id); if (!booking) throw new Error("Booking not found"); if (!["REQUESTED", "RESERVATION_PENDING"].includes(booking.status)) throw badRequest("Provisional ticket can only be uploaded for a new booking"); const saved = await saveFile(file, booking.booking_number, "provisional-ticket"); await prisma.$executeRawUnsafe(`UPDATE flight_bookings SET status='PAYMENT_PENDING', provisional_ticket_path=$2, provisional_ticket_name=$3, provisional_ticket_uploaded_at=CURRENT_TIMESTAMP, updated_at=CURRENT_TIMESTAMP WHERE id=$1`, booking.id, saved.path, saved.name); await prisma.order.update({ where: { id: booking.order_id }, data: { status: "PROCESSING", paymentStatus: "UNPAID" } }); return getFlightBooking(booking.id); }
export async function submitPaymentReceipt(id, file, phone) { const booking = await getFlightBooking(id); if (!booking) throw new Error("Booking not found"); if (!phone || normalizePhone(phone) !== normalizePhone(booking.customer_phone)) throw badRequest("بيانات التحقق غير صحيحة"); if (booking.status !== "PAYMENT_PENDING") throw badRequest("لا يمكن رفع إشعار الدفع في هذه المرحلة"); if (!(await getBankAccounts()).length) throw badRequest("لا يوجد حساب دفع نشط حاليًا"); const saved = await saveFile(file, booking.booking_number, "payment-receipt"); await prisma.$executeRawUnsafe(`UPDATE flight_bookings SET status='PAYMENT_UNDER_REVIEW', payment_receipt_path=$2, payment_receipt_name=$3, payment_receipt_uploaded_at=CURRENT_TIMESTAMP, updated_at=CURRENT_TIMESTAMP WHERE id=$1`, booking.id, saved.path, saved.name); return getFlightBooking(booking.id); }
export async function confirmPayment(id, userId, note) { const booking = await getFlightBooking(id); if (!booking) throw new Error("Booking not found"); if (booking.status !== "PAYMENT_UNDER_REVIEW") throw badRequest("Payment receipt must be reviewed before confirmation"); await prisma.$executeRawUnsafe(`UPDATE flight_bookings SET status='PAYMENT_CONFIRMED', payment_review_note=$2, updated_at=CURRENT_TIMESTAMP WHERE id=$1`, booking.id, note || null); await prisma.payment.create({ data: { orderId: booking.order_id, amount: booking.amount, currency: booking.currency, paymentMethod: "BANK_TRANSFER", status: "PAID", paidAt: new Date() } }); await prisma.order.update({ where: { id: booking.order_id }, data: { status: "PROCESSING", paymentStatus: "PAID" } }); return getFlightBooking(booking.id); }
