...
export async function uploadDocument(req, res, next) { try { const parsed = uploadContactRequestDocumentSchema.safeParse(req.body); if (!parsed.success) return res.status(400).json({ success: false, message: "Validation failed", errors: parsed.error.flatten() }); if (!req.file) return res.status(400).json({ success: false, message: "A file is required" }); const result = await uploadMyDocument(req.trackingPhone, req.params.id, { label: parsed.data.label, file: req.file }); if (result.error === "NOT_FOUND") return res.status(404).json({ success: false, message: "Contact request not found" }); return res.status(201).json({ success: true, data: result.document }); } catch (error) { next(error); } }
