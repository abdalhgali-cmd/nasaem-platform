...
export async function getOperationsCenter() {
  const [contactRequests, orders, pendingPayments, unassignedOrders, waitingDocs] = await Promise.all([
    ...
    prisma.order.findMany({ where: { status: { notIn: ["COMPLETED", "CANCELLED", "REJECTED"] } }, orderBy: { updatedAt: "asc" }, take: 50, include: { customer: { select: { fullName: true, phone: true, passportNo: true } }, assignedUser: { select: { id: true, fullName: true } }, items: { include: { service: { select: { name: true } } }, take: 1 } } }),
  ...
  const contactItems = contactRequests.map((request) => ({ source: "contact_request", id: request.id, reference: request.id, customerName: request.name, phone: request.phone, service: request.service || "استفسار", status: request.status, paymentStatus: request.paymentStatus, amount: request.invoice?.amount ?? null, currency: request.invoice?.currency ?? null, updatedAt: request.updatedAt, ageHours: Math.floor((now - new Date(request.updatedAt).getTime()) / 3600000), nextAction: contactNextAction(request) }));
  const orderItems = orders.map((order) => ({ source: "order", id: order.id, reference: order.orderNumber, customerName: order.customer?.fullName || "بدون اسم", phone: order.customer?.phone || "", service: order.items?.[0]?.service?.name || "طلب", status: order.status, paymentStatus: order.paymentStatus, amount: order.totalAmount, currency: order.currency, updatedAt: order.updatedAt, assignedUser: order.assignedUser?.fullName || null, ageHours: Math.floor((now - new Date(order.updatedAt).getTime()) / 3600000), nextAction: orderNextAction(order) }));
